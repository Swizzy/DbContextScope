...
using Mehdime.Entity.Core.Enums;
...
namespace Mehdime.Entity.Core.Implementations
{
    public class DbContextScope : IDbContextScope
    {
        ...
        public int SaveChanges()
        {
            ...
            if (_completed)
                throw new InvalidOperationException("You cannot call SaveChanges() more than once on a DbContextScope." +
                                                    ...
                                                    "create the scope at the start of the business transaction and then call SaveChanges() at the end. " +
                                                    "Calling SaveChanges() mid-way through a business transaction doesn't make sense and most likely " +
                                                    "mean that you should refactor your service method into two separate service method that each create " +
                                                    "their own DbContextScope and each implement a single business transaction.");
        ...
        public async Task<int> SaveChangesAsync(CancellationToken cancelToken)
        {
            ...
            if (_completed)
            {
                throw new InvalidOperationException("You cannot call SaveChanges() more than once on a DbContextScope." +
                                                    "A DbContextScope is meant to encapsulate a business transaction: " +
                                                    "create the scope at the start of the business transaction and then call SaveChanges() at the end. " +
                                                    "Calling SaveChanges() mid-way through a business transaction doesn't make sense and most likely mean " +
                                                    "that you should refactor your service method into two separate service method that each create their " +
                                                    "own DbContextScope and each implement a single business transaction.");
            }
            ...
            if (_nested == false)
            {
                toReturn = await CommitInternalAsync(cancelToken).ConfigureAwait(false);
            }
        ...
        public void RefreshEntitiesInParentScope<TEntity>(IEnumerable<TEntity> entities) where TEntity : class
        {
            if (entities == null)
            {
                return;
            ...
            foreach (DbContext contextInCurrentScope in _dbContexts.InitializedDbContexts.Values)
            {
                ...
                // be refreshed.
                foreach (TEntity toRefresh in entities)
                {
                    ...
                    EntityEntry stateInCurrentScope = contextInCurrentScope.Entry(toRefresh);
                    if (stateInCurrentScope != null)
                    {
                        // Now we can see if that entity exists in the parent DbContext instance and refresh it.
                        PropertyInfo primaryKeyProperty = stateInCurrentScope.Metadata.FindPrimaryKey().Properties.Single().PropertyInfo;
                        object pkValue = primaryKeyProperty.GetValue(stateInCurrentScope.Entity);
                        EntityEntry<TEntity> stateInParentScope = pkValue != null ? correspondingParentContext.ChangeTracker.Entries<TEntity>().SingleOrDefault(e => Equals(primaryKeyProperty.GetValue(e), pkValue)) : null;
        ...
        public async Task RefreshEntitiesInParentScopeAsync<TEntity>(IEnumerable<TEntity> entities) where TEntity: class
        {
            if (entities == null)
            {
                return;
        ...
        public void Dispose()
        {
            ...
            // Pop our self from the ambient scope stack
...
" + Environment.StackTrace;
        ...
        internal static void SetAmbientScope(DbContextScope newAmbientScope)
        {
            ...
            InstanceIdentifier current = CallContext.LogicalGetData(AmbientDbContextScopeKey) as InstanceIdentifier;
